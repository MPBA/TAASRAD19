...
// 64 * 64
let sprite_side = 32
let sprite_number = 120
let sprite_image_size = 64
...
let mnist_tile_string = 'meteotn_tile_solid_'
let mnist_tile_locations = [...Array(sprite_number)].map(
  (n, i) => `${process.env.PUBLIC_URL}/tiles/${mnist_tile_string}${i}.png`
)
...
class Projection extends Component {
  ...
  setUpCamera() {
    let { width, height, umap_dims_0_2 } = this.props

    let aspect = this.camera.aspect
    let vFOV = this.camera.fov
    let rvFOV = THREE.Math.degToRad(vFOV)

    let xs = umap_dims_0_2.map(e => e[0])
    let min_x = _.min(xs)
    let max_x = _.max(xs)
    let ys = umap_dims_0_2.map(e => e[1])
    let min_y = _.min(ys)
    let max_y = _.max(ys)
  ...
  addPoints() {
    let { umap_dims_0_2, mnist_labels, color_array } = this.props
    ...
    let embedding_chunks = ranges.map(range =>
      umap_dims_0_2.slice(range[0], range[1])
    )
    ...
    for (let c = 0; c < sprite_number; c++) {
      ...
      // uniforms
      console.log("texture_subsize", texture_subsize)
      let uniforms = {
      ...
      let fragment_shader = `
        ...
        void main() {
          ...
          // tex.a = 0.1;
          tex.r = tex.r + tex.r + 0.3;
          tex.g = tex.g + tex.g + 0.3;
          tex.b = tex.b + tex.b + 0.3;
  ...
  checkIntersects(mouse_position) {
    ...
    function draw_sidebar_canvas(intersects, sortIntersectsByDistanceToRay, sidebar_ctx, sidebar_image_size, offset, thi) {
      if (intersects[0]) {
        let sorted_intersects = sortIntersectsByDistanceToRay(intersects)
        let intersect = sorted_intersects[0]
        let sprite_index = intersect.object.userData.sprite_index
        let digit_index = intersect.index + offset * 2
        let full_index = sprite_index * sprite_size + digit_index
        if (offset == 0){
          thi.props.setHoverIndex(full_index)
          thi.highlightPoint(sprite_index, digit_index, full_index)
        }
        thi.scene.children[1].visible = true
        sidebar_ctx.globalCompositeOperation = "source-over";
        sidebar_ctx.drawImage(mnist_images[sprite_index],
          // source rectangle
          (digit_index % sprite_side) * sprite_image_size, Math.floor(digit_index / sprite_side) * sprite_image_size, sprite_image_size, sprite_image_size,
          // destination rectangle
          0, 0, sidebar_image_size, sidebar_image_size)
        sidebar_ctx.globalCompositeOperation = "color";
        sidebar_ctx.fillStyle = `rgb(${thi.props.color_array[thi.props.mnist_labels[full_index]].join(',')})`;
        sidebar_ctx.fillRect(0, 0, sidebar_image_size, sidebar_image_size)
        // sidebar_ctx.globalAlpha = 0.4;
      }
      else {
        sidebar_ctx.globalCompositeOperation = "source-over";
        thi.props.setHoverIndex(null);
        thi.scene.children[1].visible = false;
        sidebar_ctx.fillStyle = "black";
        sidebar_ctx.fillRect(0, 0, sidebar_image_size, sidebar_image_size)
      }
    }
    ...
    draw_sidebar_canvas(intersects, sortIntersectsByDistanceToRay, sidebar_ctx, sidebar_image_size, 0, this)
    draw_sidebar_canvas(intersects, sortIntersectsByDistanceToRay, sidebar_ctx_an1, sidebar_image_size, 1, this)
    draw_sidebar_canvas(intersects, sortIntersectsByDistanceToRay, sidebar_ctx_an2, sidebar_image_size, 2, this)
    draw_sidebar_canvas(intersects, sortIntersectsByDistanceToRay, sidebar_ctx_an3, sidebar_image_size, 3, this)
    draw_sidebar_canvas(intersects, sortIntersectsByDistanceToRay, sidebar_ctx_an4, sidebar_image_size, 4, this)
    draw_sidebar_canvas(intersects, sortIntersectsByDistanceToRay, sidebar_ctx_an5, sidebar_image_size, 5, this)
    draw_sidebar_canvas(intersects, sortIntersectsByDistanceToRay, sidebar_ctx_an6, sidebar_image_size, 6, this)
    draw_sidebar_canvas(intersects, sortIntersectsByDistanceToRay, sidebar_ctx_an7, sidebar_image_size, 7, this)
    draw_sidebar_canvas(intersects, sortIntersectsByDistanceToRay, sidebar_ctx_an8, sidebar_image_size, 8, this)
    draw_sidebar_canvas(intersects, sortIntersectsByDistanceToRay, sidebar_ctx_an9, sidebar_image_size, 9, this)
    draw_sidebar_canvas(intersects, sortIntersectsByDistanceToRay, sidebar_ctx_an10, sidebar_image_size, 10, this)
    draw_sidebar_canvas(intersects, sortIntersectsByDistanceToRay, sidebar_ctx_an11, sidebar_image_size, 11, this)
    draw_sidebar_canvas(intersects, sortIntersectsByDistanceToRay, sidebar_ctx_an12, sidebar_image_size, 12, this)
  }
